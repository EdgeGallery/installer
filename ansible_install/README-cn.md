...
### 3.3. 部署涉及的密码配置
  ...
  部署所需密码在文件`/home/EdgeGallery-v1.2.0-all-x86/install/password-var.yml`中。 **部署脚本自身不提供密码默认值，需要用户在安装前自行设定密码值。密码必须同时使用大小写字母、数字和特殊符号组合，长度不小于8位，否则会因为密码复杂度无法满足要求而造成部署失败** 。
